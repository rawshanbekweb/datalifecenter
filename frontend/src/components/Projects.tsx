...
const CATS: string[] = ['All','Web App','Mobile','Security','AI/ML','Data'];
...
function ProjCard({ p, i }: ProjCardProps): React.ReactElement {
    ...
    <motion.div initial={{ opacity:0, y:28 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true, margin:'-30px' }}
        ...
        <AnimatePresence>
          {hover && (
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
              ...
              {([Eye, GitBranch, ExternalLink] as React.ElementType[]).map((Icon: React.ElementType, idx: number) => (
      ...
      <div style={{ padding:'16px 18px' }}>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:5, marginBottom:12 }}>
          {p.tags.map((t: string) => <span key={t} style={{ fontSize:10, padding:'2px 8px', borderRadius:12, background:'#fff', color:'#64748b', border:'1px solid #e2e8f0', fontFamily:'JetBrains Mono,monospace' }}>{t}</span>)}
        </div>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingTop:10, borderTop:`1px solid ${p.border}` }}>
          <div style={{ display:'flex', gap:16 }}>
            {Object.entries(p.stats).map(([k, v]: [string, string]) => (
...
export default function Projects(): React.ReactElement {
  ...
  const list: ProjectItem[] = active === 'All' ? PROJS : PROJS.filter((p: ProjectItem) => p.cat === active);
  return (
