...
import {
  getCourseByIdAdmin, createModule, updateModule, deleteModule,
...
export default function AdminCurriculumPage(): React.ReactElement {
  ...
  const saveModule = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      if (moduleForm.id) {
        ...
        await createModule({ courseId: id, title: moduleForm.title, description: moduleForm.description || null });
      }
      setModuleForm(null);
      load();
    } catch (err: unknown) {
  ...
  return (
    <div>
      <Link to="/admin/courses" style={{ display:'inline-flex', alignItems:'center', gap:6, fontSize:13, color:'#64748b', textDecoration:'none', marginBottom:14 }}>
        <ArrowLeft size={14}/> Kurslar ro'yxati
      </Link>
